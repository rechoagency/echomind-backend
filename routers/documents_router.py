...
class DocumentStoreRequest(BaseModel):
    """Request model for document chunk storage"""
    client_id: str
    filename: str
    file_type: str
    content_text: str
...
async def store_document_chunk(request: DocumentStoreRequest) -> Dict[str, Any]:
    """
    Store a document chunk (used by N8N Document Vectorization workflow)

    This endpoint receives pre-chunked text from N8N and stores it.
    The N8N workflow handles the actual vectorization.

    Args:
        request: DocumentStoreRequest with client_id, filename, file_type, content_text

    Returns:
        Success status with chunk info
    """
    try:
        supabase = get_supabase_client()

        # Store the chunk metadata (embeddings are generated by N8N separately)
        chunk_record = {
            'client_id': request.client_id,
            'chunk_text': request.content_text,
            'metadata': {
        ...
        # Note: The actual embedding storage happens in the N8N workflow's later step
        # This endpoint just acknowledges receipt of the chunk

        logger.info(f"Received document chunk for client {request.client_id}: {request.filename}")

        return {
            'success': True,
            'client_id': request.client_id,
            'filename': request.filename,
            'chunk_length': len(request.content_text),
            'message': 'Chunk received, proceeding to embedding generation'
        }
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to store document chunk: {str(e)}"
        )
